...
int indexCollidingWith(int n, const vector<Cart> &carts) {
    for (int i = 0; i < carts.size(); ++i) {
        if (i != n && carts[n].x == carts[i].x && carts[n].y == carts[i].y) {
            return i;
        }
    }
...
Cart solve(const vector<string> &map, vector<Cart> &carts) {
    while (true) {
        ...
        while (i < carts.size()) {
            carts[i].move(map);
            int collidingIndex = indexCollidingWith(i, carts);
            if (collidingIndex >= 0) {
                carts.erase(carts.begin() + i);
                if (collidingIndex < i) {
                    carts.erase(carts.begin() + collidingIndex);
                    --i;
                } else {
                    carts.erase(carts.begin() + (collidingIndex - 1));
                }
            } else {
                ++i;
            }
